                ...
                if (!bFoundAdr && bHasStreet && (bHasZip || bHasCS))
                {
                    ...
                    do
                    {
                        ...
                        if (bHasZip && !bTrySCS)
                        {
                            if (zip != "11111")
                            {
                                bool IsTrim = zip.Length > 5;
                                var TrimZip = IsTrim ? zip.Substring(0, 5) : zip;
                                var adrL = from adr in ce.Address
                                           where (adr.ZipCode.StartsWith(TrimZip))
                                           select adr;
                                foreach (Address a in adrL)
                                {
                                    if (IsTrim && (a.ZipCode.Length > 5) && (a.ZipCode != zip))
                                        continue;
                                    adrList.Add(a);
                                }
                            }
    ...
    public class PersonMgr
    {
        ...
        public int FindPersonIDByName(IQueryable<Person> psnL, ref Person person, bool bMerge, out int address_id, bool bCheckLastName= true, Person child=null)
        {
            ...
            if (psnL.Count() > 0)
            {
                ...
                {
                    if (person.Gender.HasValue && (p.Gender.HasValue) && (person.Gender != 0) && (p.Gender != 0) && (person.Gender != p.Gender))
                        continue;
